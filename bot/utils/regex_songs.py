...
regex_songs_commands.append(('^.{0,2}don\'?t hurt me.{0,4}$', \
  baby_dont_hurt_me_resp,re.M|re.I))
...
regex_songs_commands.append(('^.{0,2}sweet dreams.{0,15}$', \
  sweet_dreams_resp,re.M|re.I))

### pirates
async def yarr_harr_resp(message: discord.Message):
  await message.channel.send('fiddle de dee')
  
regex_songs_commands.append(('^.{0,2}(yarr har|yar har).{0,4}$', \
  yarr_harr_resp,re.M|re.I))
...
regex_songs_commands.append(('^.{0,2}trust me.{0,4}$', \
  trust_me_resp,re.M|re.I))
...
regex_songs_commands.append(('^.{0,2}long ass? f(ucking)? time ago.{0,4}$', \
  long_ass_time_resp,re.M|re.I))  
...
regex_songs_commands.append(('^testing,? ["\']?attention,? please!?["\']?$', \
  testing_attention_resp,re.M|re.I))  

async def testing_emn_resp(message: discord.Message):
  await message.channel.send("**♬ Attention please! ♬**")
  
regex_songs_commands.append(('^testing.{0,4}$', \
  testing_emn_resp,re.M|re.I))  

async def spaghetti_resp(message: discord.Message):
  await message.channel.send("*♬ mom's spaghetti ♬*")
  
regex_songs_commands.append(('^.{18,30}already\W{0,4}$', \
  spaghetti_resp,re.M|re.I))  
...
regex_songs_commands.append(('^here comes the money.{0,4}$', \
  moneyyy_resp,re.M|re.I))
...
regex_songs_commands.append(('^.{0,2}yesterday.{0,4}$', \
  yesterday_resp,re.M|re.I))
...
regex_songs_commands.append(('^.{0,2}deja vu.{0,4}$', \
  deja_vu_resp,re.M|re.I))
...
regex_songs_commands.append(('^(some|.*\Wsome\W*)$', \
  somebody_resp,re.M|re.I))  
...
regex_songs_commands.append(('^(hard rock|.*\Whard rock\W*)$', \
  hard_rock_resp,re.M|re.I))    
...
regex_songs_commands.append(('^.{0,2}wake up.{0,4}$', \
  wake_up_resp,re.M|re.I))    
...
regex_songs_commands.append(('^.{0,2}beep boop.{0,4}$', \
  beep_boop_resp,re.M|re.I))
