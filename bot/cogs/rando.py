...
class Rando(commands.Cog):
    ...
    async def roll(self, interaction, dice_roll: str = SlashOption(description="Your dice roll")):
    ...
    async def choose(self, interaction,
                     choices: str = SlashOption(name="options", description="The options to choose from "
        ...
        if len(options) == 1:
            ...
            phrases = [f"Let's go with {choice}", f"I choose {choice}", f"God has spoken to me - it must be {choice}",
                       ...
                       f"{choice} all day long", f"Obviously {choice}", f"9 out of 10 dentists recommend {choice}"]
